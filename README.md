...
3. **Новый читатель формата.** Класс `JsonDataReader` - наследник `DataReader`, читает данные из JSON. Покрыт тестами в `test/test_JsonDataReader.py`. Разработка выполнена в отдельной ветке с последующим слиянием в `main`.
4. **Расчётная процедура.** Класс `DebtCounter` подсчитывает количество студентов с академическими задолженностями (балл < 61 хотя бы по одному предмету) и формирует их список. Покрыт тестами в `test/test_DebtCounter.py`. Разработка выполнена в отдельной ветке с последующим слиянием в `main`.
